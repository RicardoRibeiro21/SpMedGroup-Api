...
SELECT M.CRM, U.NOME, E.ESPECIALIZACAO, C.NOME,C.CNPJ,C.LOGRADOURO,C.RAZAO_SOCIAL,C.CEP FROM MEDICOS M JOIN CLINICA C ON M.ID_CLINICA = C.ID JOIN USUARIOS U ON M.ID_USUARIO = U.ID
...
SELECT M.CRM, U.NOME AS MEDICO, C.STATUS_CONSULTA, SC.SITUACAO, C.ID_PRONTUARIO, C.RESULTADO, P.CPF AS CPF_CLIENTE, P.RG AS RG_CLIENTE FROM MEDICOS M JOIN CONSULTAS C ON M.CRM = C.CRM_MEDICO JOIN PRONTUARIOS P ON C.ID_PRONTUARIO = P.ID JOIN USUARIOS U ON U.ID = M.ID_USUARIO JOIN STATUS_CONSULTA SC ON SC.ID = C.STATUS_CONSULTA 
